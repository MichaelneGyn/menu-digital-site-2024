...
export async function getRestaurantBySlug(slug: string): Promise<ClientRestaurant | null> {
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3001'}/api/cardapio/${slug}`, {
      cache: 'no-store' // Ensure fresh data
    });

    if (!response.ok) {
      console.error('Error fetching restaurant:', response.status, response.statusText);
      return null;
    }
